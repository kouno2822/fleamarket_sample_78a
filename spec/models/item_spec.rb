...
describe Item do
  describe '#create' do
    ...
    it "is invalid without a sell_or_sold" do
      ...
      item.valid?
      expect(item.errors[:sell_or_sold]).to include("can't be blank")
    end
    ...
    it "is valid with all must items" do
      item = build(:item)
      expect(item).to be_valid
    end
    ...
    it "is invalid with a brand that has more than 41 characters" do
      ...
      item.valid?
      expect(item.errors[:brand]).to include("is too long (maximum is 40 characters)")
    end
